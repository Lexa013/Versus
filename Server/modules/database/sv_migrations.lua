...
local function RunMigration(version)
    ...
    local migration_file = File("Packages/" ..Package.GetPath().. "/" ..migrations_folder.. "/" ..migrations[version])
...
function VS.SQL.Migrate()
    ...
    db:Select(" SELECT * FROM vs_migrations LIMIT 1;", function(data)
        VS.SQL.OnVersionRetrieved(data)
    end)
end
